---
title: "2017 Crescent Yacht Club Regatta"
...
#time labels
time_labs <- seq(round(head(perf$TIME.5S, 1), units="mins"), round(tail(perf$TIME.5S, 1), units="hour"), by="30 mins")
...
ggmap(map) + 
  geom_path(data=perf, aes(x=LON, y=LAT), color="red", linetype=1, size=1) +
  scale_color_gradientn(colors=rainbow(5), limits=c(0,10), breaks=seq(0,10, 2))+
  #geom_text_repel(data=time_labs, 
  #                aes(x=LON, y=LAT, label=format(TIME.5S, "%H:%M")), 
  #                point.padding = unit(.8, "lines"))+
  geom_path(data=marks, aes(x=lon, y=lat), color="black", linetype=2)+
  scale_x_continuous(limits=c(min(perf$LON)-.01, max(perf$LON)+.01))+
  scale_y_continuous(limits=c(min(perf$LAT)-.01, max(perf$LAT)+.01))
...
ggplot() + 
  ...
  scale_color_gradientn(colors=rainbow(5), limits=c(0,120), breaks=seq(0,120, by=20), name= "% of Target")+
  geom_text(data=pol_labels, aes(x=BTW, y=V, label=VTW), hjust=0)
...
ggplot(perf)+geom_path(aes(x=TIME.5S, y=SOG, color=pol.perc))+
...
getdist <- function(x1,y1, x2, y2){
  ...
  dist <- sqrt((x2-x1)^2 + (y1-y2)^2)
  return(dist)
}

dist.mark.A <- getdist(perf$LON, perf$LAT, rep(marks$lon[2],nrow(perf)), rep(marks$lat[2],nrow(perf)))
